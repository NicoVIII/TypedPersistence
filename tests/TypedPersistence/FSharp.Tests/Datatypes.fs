...
let tests =
    ...
    <| testList "Datatype tests"
           [ testProp "Handles ints correctly" <| simplePropertyTest<int> dbName

             testProp "Handles strings correctly"
             <| fun (text: NonNull<string>) -> text.Get |> simplePropertyTest<string> dbName

             testProp "Handles floats correctly"
             <| fun (number: NormalFloat) -> number.Get |> simplePropertyTest<float> dbName

             testProp "Handles option ints correctly" <| simplePropertyTest<int option> dbName
             ...
             testProp "Handles int list correctly" <| simplePropertyTest<int list> dbName
             ...
             testProp "Handles int record correctly" <| simplePropertyTest<GenericRecord<int>> dbName

             testProp "Handles string record correctly"
             <| fun (textRecord: GenericRecord<NonNull<string>>) ->
                 { value = textRecord.value.Get } |> simplePropertyTest<GenericRecord<string>> dbName
             ...
             testProp "Handles int record list correctly" <| simplePropertyTest<GenericRecord<int> list> dbName

             testProp "Handles int list record correctly" <| simplePropertyTest<GenericRecord<int list>> dbName

             testProp "Handles int list record record correctly"
             <| simplePropertyTest<GenericRecord<GenericRecord<int list>>> dbName

             testProp "Handles single case union type correctly" <| simplePropertyTest<SingleCaseUnion> dbName

             testProp "Handles single case int union type correctly" <| simplePropertyTest<SingleCaseIntUnion> dbName
